...
void exibirFiltro (Carros catalogo[],int filtro);
void exibir (Carros catalogo[],int tamanho,int filtro);
...
int menu (Carros catalogo[],int tamanho)
{
    ...
    {    
        ...
        case 2:
            ...
            scanf("%d",&filtro);
             exibir(catalogo,tamanho,filtro);
...
void exibir (Carros catalogo[],int tamanho,int filtro)
{
    ...
    for (i=0; i<tamanho; i++)
    {
           if((filtro>=catalogo[i].valor) ||(filtro==0))
        {
            printf("Marca: %s\n",catalogo[i].marca);
            printf("Modelo: %s\n",catalogo[i].modelo);
...
/*void exibirFiltro (Carros catalogo[],int filtro,int tamanho)
{
    int i,meno;
    for(i=0; i<2 ;i++)
    {
